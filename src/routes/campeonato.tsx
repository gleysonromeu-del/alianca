...
export default function CampeonatoPage() {
  ...
  const partidasFinalizadas = partidas
    .filter((p) => p.status === "finalizada" || p.status === "encerrada")
    .reverse();
      ...
      <main className="mx-auto max-w-6xl space-y-16 px-4 py-12 md:py-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          {leader && (
            <p className="mt-3 text-sm text-muted-foreground">
              Líder atual: <span className="font-bold text-foreground">{leader.nome}</span>{" "}
              com <span className="font-bold text-accent">{leader.pontos} pts</span>
            </p>
          )}
        ...
        <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {!loadingTimes && times.length === 0 && (
                  <tr><td colSpan={10} className="px-4 py-10 text-center text-muted-foreground">Nenhum time cadastrado neste mês.</td></tr>
                )}
                {times.map((t, i) => {
                  const jogos = t.vitorias + t.empates + t.derrotas;
                  const sg = t.gols_pro - t.gols_contra;
                  const isFirst = i === 0 && times.length > 0;
                  const isLast = i === times.length - 1 && times.length > 1;
                  return (
                    <tr key={t.id} className={`border-t border-white/5 transition-colors hover:bg-white/5 ${isFirst ? "bg-amber-500/5" : ""} ${isLast ? "bg-red-500/5" : ""}`}>
                      <td className="px-4 py-3">
                        <span className={`inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-black ${isFirst ? "bg-amber-500/20 text-amber-400" : isLast ? "bg-red-500/20 text-red-400" : "text-muted-foreground"}`}>
                          {i + 1}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <span className="inline-block h-3 w-3 shrink-0 rounded-full" style={{ background: t.cor ?? "#FFD166" }} />
                      ...
                      <td className={`px-3 py-3 text-center font-semibold ${sg > 0 ? "text-emerald-400" : sg < 0 ? "text-red-400" : ""}`}>
                        {sg > 0 ? `+${sg}` : sg}
                      </td>
          ...
          {times.length > 1 && (
            <div className="mt-3 flex flex-wrap gap-4 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-full bg-amber-400/60" />Campeão</div>
              <div className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-full bg-red-400/60" />Paga a cerveja 🍺</div>
            </div>
          )}
        </motion.section>
        ...
        <div className="grid gap-10 md:grid-cols-2">
          <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            <h2 className="mb-5 flex items-center gap-2 text-xl font-black tracking-tight">
              <Calendar className="h-5 w-5 text-accent" /> Próximas partidas
            </h2>
                ...
                <div key={p.id} className="rounded-2xl border border-white/10 bg-white/3 p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex min-w-0 flex-1 items-center justify-end gap-2">
                      <span className="truncate text-right text-sm font-semibold">{p.time_b?.nome}</span>
                      <EscudoPlaceholder cor={p.time_b?.cor} nome={p.time_b?.nome ?? "?"} />
                    </div>
                  </div>
                  {p.data && (
                    <p className="mt-2 text-center text-xs text-muted-foreground">
                      {new Date(p.data).toLocaleDateString("pt-BR", { weekday: "short", day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}
                    </p>
                  )}
                </div>
              ))}
          ...
          <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <h2 className="mb-5 flex items-center gap-2 text-xl font-black tracking-tight">
              <CheckCircle2 className="h-5 w-5 text-emerald-400" /> Resultados
            </h2>
                ...
                return (
                  <div key={p.id} className="rounded-2xl border border-white/10 bg-white/3 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className={`flex min-w-0 flex-1 items-center justify-end gap-2 ${aVenceu ? "opacity-50" : ""}`}>
                        <span className="truncate text-right text-sm font-semibold">{p.time_b?.nome}</span>
                        <EscudoPlaceholder cor={p.time_b?.cor} nome={p.time_b?.nome ?? "?"} />
                      </div>
                    </div>
                    {p.observacoes && (
                      <p className="mt-2 text-center text-xs italic text-muted-foreground">{p.observacoes}</p>
                    )}
                  </div>
                );
              })}
        ...
        <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
          <h2 className="mb-6 flex items-center gap-2 text-2xl font-black tracking-tight">
            <Trophy className="h-6 w-6 text-accent" /> Hall da Fama
          </h2>
