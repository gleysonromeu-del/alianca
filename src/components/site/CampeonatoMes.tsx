...
// ─── Types (mapeados 100% no schema real) ───────────────────────────────────
...
export default function CampeonatoMes() {
  ...
  async function fetchCampeonato() {
    setLoading(true);
    try {
      // 1. Campeonato ativo (mais recente)
      ...
      // 2. Times do campeonato (classificação)
      const { data: timesData } = await supabase
        .from("times")
        .select(
          "id, nome, cor, pontos, vitorias, empates, derrotas, gols_pro, gols_contra, cartoes_amarelos, cartoes_vermelhos"
        )
      ...
      // 3. Partidas (com nomes dos times via join)
      const { data: partidasData } = await supabase
        .from("partidas")
        .select(
          `id, data, gols_a, gols_b, status,
      ...
      // 4. Estatísticas agregadas: soma por jogador via estatisticas_partida
      //    JOIN com jogadores + time_jogadores para trazer nome, apelido, time
      const { data: estatData } = await supabase
        .from("estatisticas_partida")
        .select(
          `jogador_id, gols, assistencias, amarelos, vermelhos, presente,
           jogadores!inner(nome_completo, apelido, foto_url),
           times!inner(nome, cor)`
        )
        .in(
          "partida_id",
          (partidasData || []).map((p: any) => p.id)
        );

      if (estatData) {
        // Agrupa por jogador_id
        ...
        for (const row of estatData as any[]) {
          ...
          if (!map[jid]) {
            map[jid] = {
              jogador_id: jid,
              nome: row.jogadores?.nome_completo ?? "",
              apelido: row.jogadores?.apelido ?? "",
              foto_url: row.jogadores?.foto_url ?? null,
              time_nome: row.times?.nome ?? "",
              time_cor: row.times?.cor ?? "#888",
              gols: 0,
              assistencias: 0,
              amarelos: 0,
              vermelhos: 0,
          ...
          if (row.presente) map[jid].partidas += 1;
        }
  ...
  const S = {
    section: {
      background: "linear-gradient(180deg,#08102a 0%,#0b1536 60%,#08102a 100%)",
      padding: "80px 0",
      fontFamily: "'Segoe UI',system-ui,sans-serif",
      position: "relative" as const,
      overflow: "hidden",
    } as React.CSSProperties,
    glow: (top: string, left: string, color: string) => ({
    ...
    // grid
    grid: { display: "grid", gridTemplateColumns: "1fr 1.7fr", gap: 18, marginBottom: 18 },
    card: {
    ...
    // CTA
    cta: {
      background: "linear-gradient(135deg,rgba(59,130,246,0.09) 0%,rgba(99,102,241,0.07) 100%)",
    ...
    ctaBtn: {
      ...
      padding: "13px 26px", fontSize: 13, fontWeight: 700, cursor: "pointer",
      display: "flex", alignItems: "center", gap: 7, flexShrink: 0,
    },
    // Modal
    overlay: {
      position: "fixed" as const, inset: 0, background: "rgba(4,9,28,0.93)",
      zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center",
      padding: 20, backdropFilter: "blur(5px)",
    },
    modal: {
      background: "#0d1435", border: "1px solid rgba(255,255,255,0.1)",
      borderRadius: 20, width: "100%", maxWidth: 880,
      maxHeight: "90vh", overflowY: "auto" as const, padding: "30px 28px",
    },
    ...
    tab: (on: boolean) => ({
      ...
      border: `1px solid ${on ? "#3b82f6" : "rgba(255,255,255,0.09)"}`,
      background: on ? "rgba(59,130,246,0.14)" : "transparent",
      color: on ? "#60a5fa" : "rgba(255,255,255,0.48)",
    ...
    bar: (pct: number, cor: string) => ({
      display: "inline-block", height: 4, width: `${Math.max(4, pct * 100)}%`,
      background: cor, borderRadius: 2,
    }),
    barTrack: { width: 100, height: 4, background: "rgba(255,255,255,0.07)", borderRadius: 2, overflow: "hidden", display: "inline-block" },
    // partida card
    pCard: { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 11, padding: "14px 18px", display: "flex", alignItems: "center", gap: 16, marginBottom: 8 },
    ...
    pGol: (winner: boolean) => ({ fontSize: 19, fontWeight: 900, color: winner ? "#f59e0b" : "#fff", lineHeight: 1 }),
    pSep: { fontSize: 12, color: "rgba(255,255,255,0.25)" },
  ...
  return (
    <section style={S.section} id="campeonato">
      <div style={S.glow("8%", "4%", "rgba(59,130,246,0.07)")} />
      <div style={S.glow("auto", "auto", "rgba(245,158,11,0.05)")} />
      ...
      <div style={S.container}>
        ...
        {loading ? (
          <div style={{ textAlign: "center", color: "rgba(255,255,255,0.35)", padding: "60px 0", fontSize: 14 }}>
            Carregando campeonato...
          </div>
        ) : (
          <>
            {/* Campeão + Classificação */}
            <div style={S.grid}>
              {/* Campeão */}
              <div style={S.card}>
                <div style={S.champLabel}>
                  <Crown size={13} />
                  {campeonato?.status === "encerrado" ? "Campeão" : "Líder Atual"}
                </div>

                {campeaoTime ? (
                  <>
                    <div style={S.champRow}>
                      <div style={S.champIcon(campeaoTime.cor)}>
                        <Trophy size={26} color="#fff" />
                      </div>
                      <div>
                        <p style={S.champNome}>{campeaoTime.nome}</p>
                        <p style={S.champSub}>
                          {campeaoTime.pontos} pts · {campeaoTime.vitorias}V {campeaoTime.empates}E {campeaoTime.derrotas}D
                        </p>
                      </div>
                    </div>
                    <div style={S.statsRow}>
                      <div style={S.statBox}>
                        <div style={S.statVal()}>{campeaoTime.vitorias}</div>
                        <div style={S.statLbl}>Vitórias</div>
                      </div>
                      <div style={S.statBox}>
                        <div style={S.statVal()}>{campeaoTime.gols_pro}</div>
                        <div style={S.statLbl}>Gols</div>
                      </div>
                      <div style={S.statBox}>
                        <div style={S.statVal(true)}>{campeaoTime.pontos}</div>
                        <div style={S.statLbl}>Pontos</div>
                      </div>
                    </div>
                  </>
                ) : (
                  <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 13 }}>Nenhum time cadastrado ainda.</p>
                )}
              </div>
              ...
              <div style={S.card}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {times.map((t, i) => (
                      <tr key={t.id}>
                        ...
                        <td style={{ ...S.td, textAlign: "left" }}>
                          <span style={S.tNome}><TeamDot cor={t.cor} />{t.nome}</span>
                        </td>
                    ...
                    {times.length === 0 && (
                      <tr><td colSpan={7} style={{ ...S.td, color: "rgba(255,255,255,0.25)", paddingTop: 28 }}>Nenhum time cadastrado.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
          ...
          <div style={S.modal}>
            <div style={S.mHead}>
              <div>
                ...
                <p style={S.mSub}>{mesLabel.charAt(0).toUpperCase() + mesLabel.slice(1)}</p>
              </div>
            ...
            {aba === "artilharia" && (
              <table style={S.etbl}>
                <thead><tr>
                  ...
                  <th style={S.eth(true)}>Progresso</th>
                </tr></thead>
                <tbody>
                  {artilheiros.filter((j) => j.gols > 0).map((j, i) => (
                    <tr key={j.jogador_id}>
                      ...
                      <td style={S.etd(true)}>
                        <div style={S.barTrack}><div style={S.bar(j.gols / maxGols, "#f59e0b")} /></div>
                      </td>
                    </tr>
                  ))}
                  {artilheiros.filter((j) => j.gols > 0).length === 0 && (
                    <tr><td colSpan={6} style={{ ...S.etd(), color: "rgba(255,255,255,0.25)", paddingTop: 28 }}>Nenhum gol registrado ainda.</td></tr>
                  )}
                </tbody>
              </table>
            )}
                ...
                <tbody>
                  {assistentes.filter((j) => j.assistencias > 0).map((j, i) => (
                    <tr key={j.jogador_id}>
                      ...
                      <td style={S.etd(true)}>
                        <div style={S.barTrack}><div style={S.bar(j.assistencias / maxAssist, "#60a5fa")} /></div>
                      </td>
                    </tr>
                  ))}
                  {assistentes.filter((j) => j.assistencias > 0).length === 0 && (
                    <tr><td colSpan={6} style={{ ...S.etd(), color: "rgba(255,255,255,0.25)", paddingTop: 28 }}>Nenhuma assistência registrada ainda.</td></tr>
                  )}
                </tbody>
              </table>
            )}
            ...
            {aba === "partidas" && (
              <div>
                {partidas.length === 0 && (
                  <p style={{ color: "rgba(255,255,255,0.28)", fontSize: 13, textAlign: "center", paddingTop: 28 }}>Nenhuma partida encerrada ainda.</p>
                )}
                {partidas.map((p) => {
                  const aWin = p.gols_a > p.gols_b;
                  const bWin = p.gols_b > p.gols_a;
                  const dataFmt = p.data
                    ? new Date(p.data).toLocaleDateString("pt-BR", { day: "2-digit", month: "short" })
                    : "—";
                  return (
                    <div key={p.id} style={S.pCard}>
                      <div style={S.pRound}>{dataFmt}</div>
                      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                        ...
                        <div style={S.pScore}>
                          <span style={S.pGol(aWin)}>{p.gols_a}</span>
                          <span style={S.pSep}>×</span>
                          <span style={S.pGol(bWin)}>{p.gols_b}</span>
                        </div>
            ...
            {aba === "cartoes" && (
              <table style={S.etbl}>
                ...
                <tbody>
                  ...
                  {cartoeiros.length === 0 && (
                    <tr><td colSpan={5} style={{ ...S.etd(), color: "rgba(255,255,255,0.25)", paddingTop: 28 }}>Nenhum cartão registrado ainda.</td></tr>
                  )}
                </tbody>
              </table>
            )}
